...
async function seed() {
	try {
		// Lire et exécuter le schéma SQL pour créer les tables
		...
		if (parseInt(authorsCheck.rows[0].count) === 0) {
			...
			`);
			console.log("✅ Auteurs insérés");
		} else {
			console.log("⏭️  Auteurs déjà présents, skip");
		}
		...
		if (parseInt(booksCheck.rows[0].count) === 0) {
			...
			console.log("⏭️  Livres déjà présents, skip");
		}
		...
		if (parseInt(loansCheck.rows[0].count) === 0) {
			...
			console.log("⏭️  Emprunts déjà présents, skip");
		}
